...
<style>
/* compact styling similar to your previous layout */
...
.btn-sm { padding:4px 8px; font-size:14px; border-radius:4px; }
</style>
...
<div class="container-sheet">
            ...
            @if($isLeader)
                @foreach($members as $m)
                    @php
                        ...
                        $memberName = $m->employee->emp_name ?? $m->emp_name ?? DB::table('employee_tbl')->where('emp_id',$memberId)->value('emp_name') ?? $memberId;
                    ...
                    <div class="member-block" id="member-{{ $memberId }}">
                            ...
                            @foreach($memberAssignments as $a)
                                <div class="task-row" id="task-{{ $a->id }}">
                                    <div style="display:flex; justify-content:space-between; gap:12px;">
                                        <div style="flex:1;">
                                            <div class="small"><strong>Client:</strong> {{ optional($clients->where('client_id',$a->client_id)->first())->client_company_name ?? '-' }}</div>
                                        ...
                                        <div class="row-actions" style="min-width:220px;">
                                            @if(optional($sheet->sheet_date)->isToday() && !(isset($sheet->is_locked) && $sheet->is_locked))
                        ...
                        {{-- create-task block (initially hidden) --}}
    ...
    {{-- RIGHT column: Team info + Today's Target (single, leader-editable only) --}}
    <div class="right">
        <div class="section-title">Team Details</div>

        @if($sheet)
            <div>
                <div class="small"><strong>Team:</strong> {{ optional($sheet->team)->team_name ?? 'Team '.$sheet->team_id }}</div>
            ...
            <div style="margin-top:12px;">
                ...
                <ul>
                    @foreach($members as $tm)
                        @php $e = $tm->employee ?? DB::table('employee_tbl')->where('emp_id',$tm->emp_id)->first(); @endphp
            ...
            <div style="margin-top:12px;">
                ...
                @if($isLeader && optional($sheet->sheet_date)->isToday() && !(isset($sheet->is_locked) && $sheet->is_locked))
                    <form method="POST" action="{{ route('sheet.save_day', $sheet->id) }}">
                        @csrf
...
<script>
    ...
    // add new row (only '-' shown on new rows). Starter row keeps '+'
    ...
    // Save multiple rows - sends one POST per row to /assign
    async function saveTasksForMember(memberId, sheetId){
        ...
        for(const r of rows){
            ...
            // skip empty starter row if both empty
